...
const counter_pick = (heros) => {
    ...
    heros.forEach(each_hero => {
        let selected_hero = list.find(each_list_hero => each_list_hero.name.toLowerCase() == each_hero.toLowerCase())
    ...
    score = score.filter(each => !heros.includes(each.name.toLowerCase()))
